...
public class DocumentController {
    ...
    public ResponseEntity<?> getPdfUrl(
        ...
        LogUtils.PerformanceMonitor monitor = LogUtils.startPerformanceMonitor("GET_PDF_URL");
        try {
            // 验证token并获取用户信息
            String userId = null;
            String orgTags = null;
            ...
            try {
                ...
                LogUtils.logBusiness("GET_PDF_URL", "anonymous", "Security上下文获取失败: fileName=%s", fileName);
            }
            ...
            if (userId == null && token != null && !token.trim().isEmpty()) {
                try {
                    ...
                    LogUtils.logBusiness("GET_PDF_URL", "anonymous", "Token解析失败: fileName=%s", fileName);
                }
            }

            LogUtils.logBusiness("GET_PDF_URL", userId != null ? userId : "anonymous", "接收到PDF URL请求: fileName=%s",
                    fileName);

            // 如果没有提供token或token无效，只允许获取公开文件URL
            if (userId == null) {
                Optional<FileUpload> publicFile = fileUploadRepository.findByFileNameAndIsPublicTrue(fileName);
                if (publicFile.isEmpty()) {
                    Map<String, Object> response = new HashMap<>();
                ...
                FileUpload file = publicFile.get();
                String pdfUrl = documentService.generateDownloadUrl(file.getFileMd5());

                if (pdfUrl == null) {
                    Map<String, Object> response = new HashMap<>();
                    response.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
                    response.put("message", "无法生成PDF预览链接");
                    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
                }

                Map<String, Object> response = new HashMap<>();
                response.put("code", 200);
                response.put("message", "PDF预览链接生成成功");
                response.put("data", Map.of(
                        "fileName", file.getFileName(),
                        "pdfUrl", pdfUrl,
                        "fileSize", file.getTotalSize()));
            ...
            if (targetFile.isEmpty()) {
                LogUtils.logUserOperation(userId, "GET_PDF_URL", fileName, "FAILED_NOT_FOUND");
                monitor.end("获取PDF URL失败：文件不存在或无权限访问");
                Map<String, Object> response = new HashMap<>();
                response.put("code", HttpStatus.NOT_FOUND.value());
                response.put("message", "文件不存在或无权限访问");
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(response);
            ...
            if (pdfUrl == null) {
                LogUtils.logUserOperation(userId, "GET_PDF_URL", fileName, "FAILED_GENERATE_URL");
                monitor.end("获取PDF URL失败：无法生成预览链接");
                Map<String, Object> response = new HashMap<>();
                response.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
                response.put("message", "无法生成PDF预览链接");
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
            ...
            LogUtils.logFileOperation(userId, "GET_PDF_URL", file.getFileName(), file.getFileMd5(), "SUCCESS");
            LogUtils.logUserOperation(userId, "GET_PDF_URL", fileName, "SUCCESS");
            monitor.end("PDF预览链接生成成功");
            ...
            LogUtils.logBusinessError("GET_PDF_URL", userId, "获取PDF URL失败: fileName=%s", e, fileName);
            monitor.end("获取PDF URL失败: " + e.getMessage());
            Map<String, Object> response = new HashMap<>();
            response.put("code", HttpStatus.INTERNAL_SERVER_ERROR.value());
